...
def cvarCalculation():
    ...
    handlers = {
        "1.0.0": lambda: monteCarlo.calcualteCVar(stocks, initialPortfolio, holdingPeriodInDays),
    }
    handler = handlers.get(apiVersion)
    if handler is None:
        return Response("Version not supported", status=400)

    # calcualteCVar already returns a JSON-encoded string; return it as the
    # response body directly instead of wrapping it in a dict, which would
    # make Flask JSON-encode an already-JSON string (double-encoding).
    return Response(handler(), mimetype='application/json')
